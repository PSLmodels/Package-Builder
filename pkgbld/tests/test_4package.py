...
def test_for_package_existence():
    """
    Ensure that no conda pkgbld package is installed when running pytest.
    Primarily to help developers catch mistaken installations of pkgbld;
    the local mark prevents test from running on GitHub.
    """
    out = subprocess.check_output(['conda', 'list', 'pkgbld']).decode('ascii')
    envless_out = out.replace('pkgbld-dev', 'environment')
    if re.search('pkgbld', envless_out) is not None:
        assert 'pkgbld package' == 'installed'
